...
<div class="modal fade" id="supp{{$keys->id}}" tabindex="-1" data-bs-backdrop="static"
    ...
    <div class="modal-dialog ">
        <form action="{{route('factory.delete', $keys->id)}}" method="POST">
            @csrf
            @method('patch')

            <div class="modal-content">
                <div class="modal-header">
                    <h1 class="modal-title fs-5" id="exampleModalLabel">Suppression de l'Usine
                        <strong>{{$keys->nomusine}}</strong>
                    </h1>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    Voulez-vous réellement supprimer l'<strong>{{$keys->nomusine}}</strong> ?
                    {{-- <input type="hidden" name="id" value="{{$keys->id}}"> --}}
                    <input type="hidden" name="active" value="false">
                    {{-- <input type="hidden" name="usine" value="{{$keys->id}}"> --}}
                </div>
